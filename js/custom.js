...
function FetchFromParse(Tablename,number){
    alert("In fetch");
        ...
        {       console.log(number+"skdk");
                query.equalTo("MobileNumber",number);  }
        query.limit(1);
        query.find({
                ...
                { 
                    ...
                    {   
                        for(var i=0, iL=idArr.length; i<iL;i++)
                            {
                                var key=idArr[i];
                                var value=tempcoor.get(idArr[i]);
                                if(tempcoor.get(idArr[i]))
                                        {
                                            if(key=="MobileNumber"){window.localStorage.setItem("MobileNumber",value);}
...
function JSONToCSVConvertor(JSONData, ReportTitle, ShowLabel) {
    ...
    document.body.removeChild(link);
        ...
        //var mNumber = "8904845390";
        window.localStorage.setItem("MobileNumber", mNumber);
...
function checkCallerNumber()
{
    var promise = new Promise(function(resolve, reject) {
    FetchFromParse("TEMPCOOR","1");
        resolve(true);
        console.log("TEmpok");
    });
    promise.then(function(data) { 
    if(mnum!=null)
        {
            FetchFromParse("DETAILS",mnum);
            console.log("TEmpok");
        }
    else if(window.localStorage.getItem("MobileNumber")){
        var n=window.localStorage.getItem("MobileNumber");
